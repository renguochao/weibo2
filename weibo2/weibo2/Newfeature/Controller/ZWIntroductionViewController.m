...
- (void)viewDidLoad {
    ...
    if (!self.enterButton) {
        ...
        [self.enterButton setTitle:NSLocalizedString(@"注册", nil) forState:UIControlStateNormal];
        self.enterButton.layer.borderWidth = 0.5;
        self.enterButton.layer.borderColor = [UIColor whiteColor].CGColor;
    }
    
    [self.enterButton addTarget:self action:@selector(regist:) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.loginButton.frame = [self frameOfLoginButton];
...
{
    return CGRectMake(0, self.view.bounds.size.height - 140, self.view.bounds.size.width, 30);
}
...
- (CGRect)frameOfEnterButton
{
    ...
    return CGRectMake(self.view.frame.size.width / 2 - size.width / 2, self.pageControl.frame.origin.y - 0.2 * size.height, size.width, size.height);
}

- (CGRect)frameOfLoginButton
{
    CGSize size = self.loginButton.bounds.size;
    if (CGSizeEqualToSize(size, CGSizeZero)) {
        size = CGSizeMake(self.view.frame.size.width * 0.6, 40);
    }
...
- (void)regist:(id)object
{
    self.didSelectedRegist();
}

- (void)login:(id)object
{
    self.didSelectedLogin();
}
